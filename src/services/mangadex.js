 ...
 * @param {string[]} [options.includedTags] - UUIDs de tags de MangaDex a incluir
 * @param {string[]} [options.contentRating] - por defecto ['safe','suggestive']
 * @param {AbortSignal} [options.signal] - para cancelar la petición (usado por el autocompletado)
 */
export async function searchManga(title, options = {}) {
  const {
    limit = 20,
    includedTags = [],
  ...
  const data = await smartFetchJson(url, { signal })
  return data.data.map(normalizeManga)
}

/**
 * Sugerencias en vivo para el autocompletado del buscador.
 * Reutiliza el mismo endpoint de búsqueda de MangaDex (no existe uno dedicado),
 * pero con un límite bajo para que la respuesta sea liviana y rápida.
 */
export async function getSuggestions(title, options = {}) {
  const { limit = 6, signal } = options
  return searchManga(title, { limit, signal })
}

/**
 * Lista mangas populares (usado en la Home cuando no hay búsqueda ni filtros activos,
 * y también para catálogos filtrados por género o para la zona +18).
 * @param {object} [options]
 * @param {number} [options.limit=20]
 * @param {string[]} [options.includedTags]
 * @param {string[]} [options.contentRating] - por defecto ['safe','suggestive']
...
/**
 * Lista cruda de todos los tags (géneros, temas, formato y contenido) de MangaDex.
 * Solo debe llamarse bajo demanda (primera vez que el usuario usa los filtros),
 * nunca durante la carga inicial de la Home. Ver src/services/tagCatalog.js,
 * que envuelve esta función con caché en localStorage.
 * @param {object} [options]
 * @param {AbortSignal} [options.signal]
 */
export async function fetchTagList(options = {}) {
